...
const TermIdPage = () => {
  ...
  useEffect(() => {
    http
      .sendRequest(
        `/${clientType === 'teacher' ? 'professor' : clientType}/term/${id}/`,
        { method: 'get' }
      )
      .then((res) => {
    ...
    <>
      <Container header={`${data?.name} ترم`}>
        {clientType !== 'teacher' && (
          <>
            <Link
              to={`/dashboard/${clientType}/term/${id}/preregistration_courses`}
        ...
        <Link
          to={`/dashboard/${clientType}/term/${id}/registration_courses`}
          className={classes.button}
          state={{ name: data?.name }}
        >
          دروس ثبت نامی
